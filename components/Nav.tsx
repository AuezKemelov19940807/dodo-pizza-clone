...
const Nav = () => {
  ...
  return (
    <div ref={refComponent} className="flex items-center flex-1">
      <CSSTransition
        in={scroll}
        timeout={900}
        classNames="my-icon"
      ...
      <ul
        className={`flex items-center gap-x-4 transform   ${
          scroll ? 'translate-x-[52px]' : 'translate-x-[0px]'
        } transition-all duration-300`}
      >
        {navItems.map((navItem, index) => {
          const { name, path } = navItem
          return (
            <li className="" key={`${name}_${index}`}>
              <Link
                className="cursor-pointer text-sm hover:text-primary  font-medium"
                to={path}
              >
                {name}
              </Link>
